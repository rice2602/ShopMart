...
class ShopMartPipeline:
    def __init__(self, mode="local", storage_config=None):
        ...
        if self.mode == "aws":
            ...
            self.metadata_table = self.dynamodb.Table(
                self.storage_config.get("metadata_table", "shopmart-metadata")
            )
            ...
            self.local_processed_dir = self.storage_config.get(
                "local_processed", "data/processed"
            )
            self.local_quarantine_dir = self.storage_config.get(
                "local_quarantine", "data/quarantine"
            )
            self.local_metadata_file = self.storage_config.get(
    ...
    def _log_metadata(
        self, file_name, status, record_count=0, error_reason=None, metrics=None
    ):
    ...
    def process_data(self, file_name, file_content_bytes):
        ...
        if self._check_idempotency(file_name):
            logger.info(
                f"File {file_name} was already processed successfully. Skipping."
            )
            self._log_metadata(
                file_name, "DUPLICATE", error_reason="Duplicate file upload ignored"
            )
        ...
        if len(df) == 0:
            ...
            self._log_metadata(
                ...
                "SUCCESS",
                record_count=0,
                error_reason="Empty file processed",
            )
        ...
        valid_mask = (
            ...
            & (df_clean["order_id"].astype(str).str.strip() != "")
            & df_clean["customer_id"].notna()
            & (df_clean["customer_id"].astype(str).str.strip() != "")
            & df_clean["product_id"].notna()
            & (df_clean["product_id"].astype(str).str.strip() != "")
            & df_clean["order_date"].notna()
            & (df_clean["order_date"].astype(str).str.strip() != "")
        ...
        if not df_good.empty:
            dup_mask = df_good.duplicated(
                subset=["order_id", "product_id"], keep="first"
            )
            ...
            if not df_dups.empty:
                ...
                dup_original = (
                    df[df.index.isin(df_dups.index)].copy().reset_index(drop=True)
                )
                df_bad = pd.concat(
                    [df_bad.reset_index(drop=True), dup_original], ignore_index=True
                )
        ...
        if not df_good.empty:
            ...
            payment_success_rate = (
                float(paid_orders / total_orders) if total_orders > 0 else 0.0
            )
        ...
        if not df_bad.empty:
            ...
            self._send_alert(
                file_name,
                f"Quarantined {len(df_bad)} records containing schema, duplicate, or validation errors.",
            )
        ...
        self._log_metadata(
            ...
            error_reason=(
                None if len(df_bad) == 0 else f"Quarantined {len(df_bad)} rows"
            ),
            metrics=metrics,
        )
    ...
    def _write_processed_parquet(self, file_name, df):
        ...
        if self.mode == "aws":
            ...
            try:
                date_str = str(df["order_date"].iloc[0]).split(" ")[
                    0
                ]  # extract YYYY-MM-DD
            ...
            try:
                ...
                logger.info(
                    f"Uploaded processed parquet to s3://{self.storage_config['processed_bucket']}/{s3_key}"
                )
            except Exception as e:
    ...
    def _write_quarantine_csv(self, file_name, df):
        ...
        if self.mode == "aws":
            ...
            try:
                ...
                logger.info(
                    f"Uploaded quarantined records to s3://{self.storage_config['quarantine_bucket']}/{s3_key}"
                )
            except Exception as e:
    ...
    def _write_quarantine_raw(self, file_name, file_content_bytes):
        ...
        if self.mode == "aws":
            ...
            try:
                ...
                logger.info(
                    f"Uploaded quarantined raw file to s3://{self.storage_config['quarantine_bucket']}/{s3_key}"
                )
            except Exception as e:
    ...
    def _write_metrics_summary(self, file_name, metrics):
        ...
        if self.mode == "aws":
            ...
            try:
                ...
                logger.info(
                    f"Uploaded daily metrics summary to s3://{self.storage_config['processed_bucket']}/{metrics_key}"
                )
            except Exception as e:
            ...
            output_path = os.path.join(
                metrics_dir, file_name.replace(".csv", "_metrics.json")
            )
...
def lambda_handler(event, context):
    ...
    for record in event.get("Records", []):
        ...
        try:
            ...
            logger.error(
                f"Lambda execution failed for object s3://{bucket}/{key}: {str(e)}"
            )
